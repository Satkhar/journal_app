...
боевая рука, optional месяц начала тренировок, plain-text notes и
`archived_at`. Birthday хранится как day/month и optional year; фиктивный год
не используется. Начало тренировок задаётся вручную с точностью до месяца,
...
визуальным признаком, но списки участников сортируются по отображаемому имени.
...
- `participants`: вычисленное `display_name`, optional `historical_name`,
  ...
  `training_start_month`, `notes`, `archived_at`;
- `month_participants`;
- `attendance`;
- `month_days`;
- `participant_day_markers`;
...
тренировок; v10 -> v11 добавляет герб и замеры ударов. Unversioned legacy
schema сначала мигрирует в v3, затем до v11:
...
удаляется, затем схема мигрирует через v7, v8, v9, v10 до v11.
